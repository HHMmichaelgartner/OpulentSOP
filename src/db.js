...
function toApp(row, type) {
  if (!row) return null;
  if (type === 'user') return { id: row.id, name: row.name, email: row.email || '', roleId: row.role_id, department: row.department || '', active: row.active !== false, createdAt: row.created_at, lastLogin: row.last_login };
...
function toDB(obj, type) {
  if (!obj) return null;
  if (type === 'user') return { id: obj.id, name: obj.name, email: obj.email || '', role_id: obj.roleId, department: obj.department || '', active: obj.active !== false, created_at: obj.createdAt || new Date().toISOString(), last_login: obj.lastLogin || null };
