  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        // Check if it's a ZIP (docx files are zip archives)
        if (buf[0] === 0x50 && buf[1] === 0x4B) {
          // Minimal ZIP parser - find word/document.xml
          const text = new TextDecoder().decode(buf);
          const xmlStart = text.indexOf('<?xml');
          const bodyStart = text.indexOf('<w:body');
          if (bodyStart > -1) {
            const bodyEnd = text.indexOf('</w:body>', bodyStart);
            const bodyXml = text.slice(bodyStart, bodyEnd > -1 ? bodyEnd : undefined);
            const parsed = bodyXml
              .replace(/<w:p[^>]*\/>/g, "\n")
              .replace(/<w:p[^>]*>/g, "\n")
              .replace(/<\/w:p>/g, "")
              .replace(/<w:r[^>]*>/g, "")
